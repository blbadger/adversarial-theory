...
model = FCEncoder(10000, channels=1, image_size=28).to(device) # 10000
...
def train_diffusion_inversion():
	...
	for epoch in range(epochs):
		...
		torch.save(model.state_dict(), 'fcnet_fmnist.pth')

		if epoch % 5 == 0:
			gen_images = sample(model, image_size, batch_size=batch_size, channels=channels)
			gen_images = (gen_images[-1] + 1) / 2
			show_batch(gen_images, count=epoch//5, grayscale=True, normalize=True)

model.load_state_dict(torch.load('fcnet_fmnist.pth'))
...
def generate_adversarials(lr=0.01):
	...
	adversarial_output = model(adversarial_inputs, timestep)
	adversarial_output_l1 = torch.sum(torch.abs(output - adversarial_output))
	routput = model(input_tensor + lr * torch.randn(input_tensor.shape).to(device), timestep)
